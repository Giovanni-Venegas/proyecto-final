...
class DetailViewController: UIViewController {
    ...
    private func setupContactInfo() {
        nameField.text = detailViewModel.contact.firstname ?? ""
        phoneField.text = detailViewModel.contact.phone ?? ""
        lastnameField.text = detailViewModel.contact.lastname ?? ""
    ...
    private func setupView() {
        ...
        lastnameField.addTarget(self, action: #selector(didChangeTextField), for: .editingChanged)
    }
    
    @objc func didChangeTextField(_ sender: UITextField) {
        if sender == nameField {
            detailViewModel.contact.firstname = sender.text ?? ""
        }
        ...
        if sender == lastnameField {
            detailViewModel.contact.lastname = sender.text ?? ""
        }
    }
